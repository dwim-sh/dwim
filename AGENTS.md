...
* `harness/`: the agent around the model: runs the tools it calls and feeds the results back until it replies with text alone. The system prompt declares the tools in the form the model's chat template uses: their JSON signatures, and calls as `<function=…>` blocks. Tool results are encoded as plain text, and only the chat's own framing as special tokens, so a command's output cannot end a turn.
